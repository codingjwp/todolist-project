...
const TodoDetail = ({
  ...
  return (
    <TodoItemBase aria-label={`todo-detail-${id}`}>
      <CheckBoxField
        aria-label={`detail-check-${id}`}
        isCompleted={isCompleted}
        onClick={() => updateTodoListApi(id, !isCompleted, todo)}
      />
      <TodoContent onClick={() => detailTodoOpen(todo)}>{todo}</TodoContent>
      <IconButton
        aria-label={`detail-edit-${id}`}
        iconName='btn-edit'
        iconFill='#ffffff'
        iconWidth='24'
        iconHeight='24'
        ...
        $isIconOfText='no'
        $btnType='primary'
        onClick={detailOfModify}
      />
      <IconButton
        aria-label={`detail-delete-${id}`}
        iconName='btn-delete'
        iconFill='#ffffff'
        iconWidth='24'
        iconHeight='24'
...
const TodoModify = ({
  ...
  return (
    <TodoItemBase aria-label={`todo-modify-${id}`}>
      <InputField
        ref={modifyRef}
        aria-label={`modify-text-${id}`}
        name={`modify-text-${id}`}
        type='text'
        defaultValue={todo}
      />
      <IconButton
        aria-label={`modify-send-${id}`}
        iconName='btn-send'
        iconFill='#ffffff'
        iconWidth='24'
        iconHeight='24'
      ...
      />
      <IconButton
        aria-label={`modify-cancel-${id}`}
        iconName='btn-send-cancel'
        iconFill='#ffffff'
        iconWidth='24'
        iconHeight='24'
        type='button'
...
const TodoItems = ({
  ...
  return change ? (
    <TodoDetail
      ...
      isCompleted={isCompleted}
      todo={todo}
      detailOfModify={detailOfModify}
      deleteTodoListApi={deleteTodoListApi}
      updateTodoListApi={updateTodoListApi}
      detailTodoOpen={detailTodoOpen}
    />
  ) : (
    <TodoModify
      id={id}
